class VideoPlayer extends HTMLElement {
  constructor() {
    ...
    shadow.innerHTML = `
                <video data-video-player="video" part="video"
                    ${this.muted && "muted"}
  ...
  events() {
    this.video.addEventListener("loadedmetadata", () => {
      ...
      if (this.durationElement)
        this.durationElement.textContent = this.convertTimeToDuration(this.duration);
      if (this.progressBar) this.progressValue;
    });
    this.video.addEventListener("timeupdate", () => {
      this.progressValue = (this.video.currentTime / this.video.duration) * 100;
      if (this.currentTimeElement)
        this.currentTimeElement.textContent = this.convertTimeToDuration(this.video.currentTime);
